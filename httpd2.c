...
#define MAX_REQUEST_BODY_LENGHT (1024 * 1024)
...
read_request(FILE *in)
{
    ...
    if (req->length != 0) {
        if (req->length > MAX_REQUEST_BODY_LENGHT)
...
content_length(struct HTTPRequest *req)
{
    ...
    if (!val) return 0;
    len = atol(val);
    if (len < 0) log_exit("negative Content-Length valeu");
    return len;
}
...
{
    output_common_header_fields(req, out, "405 Method Not Allowd");
    fprintf(out, "Content-Type: text/html\r\n");
    fprintf(out, "\r\n");
    fprintf(out, "<html>\r\n");
    fprintf(out, "<header>\r\n");
    fprintf(out, "<title>405 Method Not Allowd</title>\r\n");
    fprintf(out, "</header>\r\n");
    fprintf(out, "<body>\r\n");
...
output_common_header_fields(struct HTTPRequest *req, FILE *out, char *status)
{
    ...
    strftime(buf, TIME_BUF_SIZE, "%a, %d %b %Y %H:%M%S GM%", tm);
